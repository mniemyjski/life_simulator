...
class InstrumentScreen extends StatelessWidget implements AutoRouteWrapper {
  final String id;
  final CandlesCubit blocProvider;

  const InstrumentScreen({Key? key, required this.id, required this.blocProvider})
      : super(key: key);
        ...
        return state.maybeWhen(
            ...
            loaded: (instruments) {
              ...
              List<Candle> candlesFroI = context.watch<CandlesCubit>().state.maybeWhen(
                  orElse: () => [],
                  loaded: (candles) =>
                      candles.where((g) => g.instrument == instrument.name).toList());

              return CustomScaffold(
                appBar: AppBarGame(
                  title: Enums.toText(instrument.name).tr(),
                  showTimeSpend: false,
                ),
                body: Column(
                  children: [
                            ...
                            return state.maybeWhen(
                                ...
                                loaded: (t) {
                                  ...
                                  for (var element in transactions) {
                                    ...
                                    cost += element.count * element.instrument.lastCandle.close;
                                  }
                                              ...
                                              child: RichText(
                                                text: TextSpan(
                                                  ...
                                                  children: <TextSpan>[
                                                    ...
                                                    TextSpan(
                                                      text:
                                                          '${amount.toExp()} ${Enums.toText(instrument.name)}',
                                                    ),
                                                  ],
                                                ),
                                              ),
